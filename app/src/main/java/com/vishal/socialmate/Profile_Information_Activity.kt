    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        updateBtn.setOnClickListener {
            ...
            if(username.isEmpty()){
                ...
                val userNameQuery = databaseRef.orderByChild("username").equalTo(username)

                userNameQuery.addListenerForSingleValueEvent(object: ValueEventListener{
                    override fun onDataChange(snapshot: DataSnapshot) {
                        if(snapshot.exists()){
                            progressbar.visibility = View.GONE
                            Toast.makeText(this@Profile_Information_Activity, "Username already exists. Please choose a different one.", Toast.LENGTH_SHORT).show()
                        }else {
                            if(selectedImageUri != null){
                                    ...
                                    .addOnSuccessListener {
                                        imageRef.downloadUrl
                                            .addOnSuccessListener {  uri ->
                                                ...
                                                progressbar.visibility = View.GONE
                                                Toast.makeText(this@Profile_Information_Activity,"Profile Information Saved Successfully \uD83D\uDCA5",Toast.LENGTH_SHORT).show()

                                                val intent = Intent(this@Profile_Information_Activity,MainActivity::class.java)
                                                startActivity(intent)
                                                finish()

                                            }
                                            .addOnFailureListener {
                                                Toast.makeText(this@Profile_Information_Activity,"Failed to Upload Image: ${it.message}",Toast.LENGTH_SHORT).show()
                                                progressbar.visibility = View.GONE
                                            }
                                    }
                                    .addOnFailureListener {
                                ...
                                emailKey?.let {
                                    ...
                                    emailKey?.let { firestoreRef.document(it).set(defUserprofile)
                                        ...
                                        .addOnFailureListener {
                                            Toast.makeText(this@Profile_Information_Activity,"Failed to Update Information",Toast.LENGTH_SHORT).show()
                                            progressbar.visibility = View.GONE
                                        }
                                    }
                    ...
                    override fun onCancelled(error: DatabaseError) {
                        progressbar.visibility = View.GONE
                        Toast.makeText(this@Profile_Information_Activity,"Failed to check UserName",Toast.LENGTH_SHORT).show()
                    }
                })
            }
        }
